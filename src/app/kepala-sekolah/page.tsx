...
export default function KepsekDashboard() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (res.data?.data) {
        ...
        throw new Error(
          "Data format tidak sesuai. Response tidak mengandung field 'data'",
        );
      }
    } catch (err: any) {
      const errorMessage =
        err.response?.statusText || err.message || "Koneksi gagal";
      ...
      if (statusCode === 500) {
        setError(
          "🔴 Server Error (500). Backend API sedang bermasalah. Hubungi administrator.",
        );
      } else if (statusCode === 401) {
        setError("❌ Unauthorized. Silakan login kembali.");
      } else if (statusCode === 404) {
        setError("❌ Endpoint tidak ditemukan (404).");
      } else if (err.code === "ECONNABORTED") {
        setError(
          "⏱️ Request timeout. Jaringan lambat atau server tidak merespons.",
        );
      ...
      <KepsekLayout>
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="text-center max-w-md">
            ...
            <div className="mt-4 p-3 bg-slate-100 rounded text-sm text-left">
              <p className="font-semibold text-slate-700 mb-2">
                💡 Tips Debug:
              </p>
              <ul className="text-slate-600 text-xs space-y-1">
                <li>• Pastikan server backend sedang berjalan</li>
                <li>• Cek ngrok tunnel masih aktif</li>
                <li>• Buka F12 → Console untuk error detail</li>
                <li>• Attempt: {retryCount}</li>
              </ul>
            </div>
          </div>
        </div>
  ...
  const todayTotal = Object.values(data?.attendance_today || {}).reduce(
    (a: any, b: any) => a + b,
    0,
  ) as number;
  const todayHadir =
    (data?.attendance_today?.Hadir || 0) +
    (data?.attendance_today?.Terlambat || 0);
  const todayPct =
    todayTotal > 0 ? Math.round((todayHadir / todayTotal) * 100) : 0;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6">
          <div className="flex justify-between items-center mb-5">
            ...
            <span
              className={`text-xs font-black px-3 py-1 rounded-full ${todayPct >= 80 ? "bg-emerald-100 text-emerald-700" : todayPct >= 60 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700"}`}
            >
              {todayPct}% Hadir
            </span>
          </div>
          ...
          <div className="space-y-2.5">
            {(data?.trend_7_days || []).map((day: any, i: number) => (
              <div key={i} className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-slate-400 w-20 shrink-0">
                  {day.day_label}
                </span>
                ...
                <span
                  className={`text-[10px] font-black w-10 text-right ${day.persentase >= 80 ? "text-emerald-600" : day.persentase >= 60 ? "text-amber-600" : "text-red-600"}`}
                >
                  {day.persentase}%
                </span>
              </div>
          ...
          <div className="space-y-3">
            {(data?.guru_terendah || []).length > 0 ? (
              data.guru_terendah.map((guru: any, i: number) => (
                <div
                  key={guru.id}
                  className="flex items-center gap-3 p-3 hover:bg-slate-50 rounded-2xl transition-all"
                >
                  <div
                    className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-black ${i === 0 ? "bg-red-100 text-red-600" : i === 1 ? "bg-orange-100 text-orange-600" : "bg-amber-100 text-amber-600"}`}
                  >
                    {i + 1}
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    <span
                      className={`text-sm font-black ${guru.persentase >= 75 ? "text-amber-600" : "text-red-600"}`}
                    >
                      {guru.persentase}%
                    </span>
                    <p className="text-[10px] text-slate-400">
                      {guru.hadir + guru.terlambat}/{guru.total} sesi
                    </p>
                  </div>
                </div>
              ))
          ...
          <div className="space-y-3">
            {(data?.recent_leaves || []).length > 0 ? (
              data.recent_leaves.map((leave: any) => (
                <div
                  key={leave.id}
                  className="flex items-start gap-3 p-3 hover:bg-slate-50 rounded-2xl transition-all border border-transparent hover:border-slate-100"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] text-slate-400">
                      {leave.type} • {leave.start_date} s/d {leave.end_date}
                    </p>
                    <p className="text-[10px] text-slate-300 mt-0.5">
                      Diajukan {leave.created_at}
                    </p>
                  </div>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            ...
            <p className="text-indigo-100 text-sm max-w-md">
              Laporan rekapan absensi semua guru kini bisa diunduh dalam bentuk
              cetak Excel maupun PDF langsung melalui sistem.
            </p>
          </div>
          <div className="shrink-0">
            <Link
              href="/kepala-sekolah/reports"
              className="bg-white text-indigo-600 px-6 py-3 rounded-xl font-bold text-sm shadow-lg hover:bg-slate-50 active:scale-95 transition-all inline-flex items-center gap-2"
            >
