...
export default function AdminSchedules() {
  ...
  const fetchDropdowns = async (retryCount = 0) => {
    try {
      console.log("📥 Fetching dropdown data...");
      const [uRes, rRes] = await Promise.all([
        api.get("/admin/schedules-users", { timeout: 20000 }),
        api.get("/admin/schedules-rooms", { timeout: 20000 }),
      ]);
      setUsersList(uRes.data.data);
      setRoomsList(rRes.data.data);
      console.log("✅ Dropdown data loaded");
    } catch (err: any) {
      console.error("❌ Failed to fetch dropdowns:", err.message);

      // Retry maksimal 2 kali jika timeout
      if (
        retryCount < 2 &&
        (err.code === "ECONNABORTED" || err.message.includes("timeout"))
      ) {
        console.log(`🔄 Retry attempt ${retryCount + 1}...`);
        setTimeout(() => fetchDropdowns(retryCount + 1), 2000);
      }
    }
  };

  const fetchSchedules = async (page = 1, retryCount = 0) => {
    setLoading(true);
    try {
      console.log("📥 Fetching schedules...");
      const res = await api.get(
        `/admin/schedules?page=${page}&user_id=${userFilter}&day=${dayFilter}`,
        {
          timeout: 20000,
        },
      );
      ...
      if (
        ...
        console.log(`🔄 Retry attempt ${retryCount + 1}...`);
        setTimeout(() => fetchSchedules(page, retryCount + 1), 2000);
        return;
      }
    } finally {
      setLoading(false);
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm bg-white inline-block">
            Kelola jadwal kelas / penugasan guru
          </p>
        </div>
        <button
          ...
          <div className="p-4 border-t border-slate-100 flex justify-between items-center text-xs text-slate-500 font-medium">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-3 py-1 bg-slate-100 hover:bg-slate-200 rounded-lg disabled:opacity-50"
              >
                Prev
              </button>
              <button
