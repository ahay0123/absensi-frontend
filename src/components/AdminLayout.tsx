...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex">
            ...
            {/* Sidebar */}
            <aside className={`fixed lg:sticky top-0 left-0 h-screen w-64 bg-white border-r border-slate-100 flex flex-col transition-transform duration-300 z-50 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
                <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                    <div>
                        <h2 className="font-black text-xl text-slate-800 tracking-tight">Admin<span className="text-indigo-600">Panel</span></h2>
                        <p className="text-[10px] text-slate-400 font-medium tracking-widest uppercase">Absensi Guru</p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                    {navs.map((nav) => {
                        ...
                        return (
                            ...
                            >
                                <nav.icon className={`w-5 h-5 ${isActive ? "text-indigo-600" : "text-slate-400"}`} />
                                {nav.name}
                            </Link>
                        );
                    })}
                ...
                <div className="p-4 border-t border-slate-100">
                    <div className="flex items-center gap-3 mb-4 px-2">
                        <div className="w-10 h-10 rounded-full bg-indigo-100 overflow-hidden">
                            <img src={user.avatar || `https://ui-avatars.com/api/?name=${user.name}&background=e0e7ff&color=4f46e5`} alt="avatar" />
                        </div>
            ...
            <main className="flex-1 flex flex-col min-w-0 min-h-0 overflow-auto">
                ...
                <header className="lg:hidden bg-white border-b border-slate-100 p-4 flex items-center gap-4 sticky top-0 z-30">
                    <button onClick={() => setIsSidebarOpen(true)} className="p-2 -ml-2 text-slate-600 bg-slate-50 rounded-lg">
